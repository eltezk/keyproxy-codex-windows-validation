...
O instalador:
  ...
  5. cria backups, valida o TOML e executa codex logout;
...
resolve_file_symlink() {
  ...
  while [[ -L "$path" ]]; do
    ((depth += 1))
    ((depth <= 20)) || die "Muitos links simbólicos ao resolver: $1"
    target="$(readlink "$path")" || die "Não foi possível ler o link simbólico: $path"
    if [[ "$target" == /* ]]; then
      path="$target"
    else
...
download_file() {
  ...
  if command -v curl >/dev/null 2>&1; then
    ...
    die "Instale curl ou wget para baixar o Codex CLI."
  fi
}
...
main() {
  ...
  for command_name in uname id awk grep sed date mktemp cp mv rm chmod mkdir dirname cat readlink; do
  ...
  CONFIG_FILE="$(resolve_file_symlink "$CODEX_CONFIG_HOME/config.toml")"
  SHELL_PROFILE="$(resolve_file_symlink "$SHELL_PROFILE")"
